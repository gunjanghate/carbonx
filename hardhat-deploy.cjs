...
task("deploy-mock", "Deploy mock contracts for testing")
    .setAction(async (taskArgs, hre) => {
        ...
        // Deploy second contract (Carbon LedgerToken mock)
        console.log('💎 Deploying Carbon LedgerToken mock...');
        const Carbon Ledger = await StorageContract.deploy();
        await Carbon Ledger.waitForDeployment();
        const Carbon LedgerAddress = await Carbon Ledger.getAddress();
        console.log(`✅ Carbon LedgerToken deployed at: ${Carbon LedgerAddress}`);
        ...
        updateEnv('NEXT_PUBLIC_CARBON_X_TOKEN_ADDRESS', Carbon LedgerAddress);
        updateEnv('NEXT_PUBLIC_CONTRACT_ADDRESS', carbonCreditAddress);
        updateEnv('NEXT_PUBLIC_ERC20_ADDRESS', Carbon LedgerAddress);
        ...
        console.log(`💎 Carbon LedgerToken: ${ Carbon LedgerAddress }`);
        console.log('✅ Environment variables updated');
        
        return {
            carbonCreditAddress,
            Carbon LedgerAddress
        };
    });
