...
const Carbon Ledger_KNOWLEDGE_BASE = `
Carbon Ledger Platform Knowledge Base:

ABOUT Carbon Ledger:
Carbon Ledger is a comprehensive platform for carbon credit trading, sustainability tracking, and environmental impact management. We provide AI-powered tools to help individuals and businesses calculate, track, and offset their carbon footprint while participating in the carbon credit marketplace.
...
const FALLBACK_RESPONSES = {
  'what is Carbon Ledger': 'Carbon Ledger is a comprehensive platform for carbon credit trading, sustainability tracking, and environmental impact management. We provide AI-powered tools to help calculate, track, and offset your carbon footprint.',
  'ai tools': 'Carbon Ledger offers several AI-powered tools: AI Carbon Calculator for emission analysis, AI Plastic Footprint calculator for waste tracking, and Sustainable Event Planner for eco-friendly event management.',
  ...
  'security': 'Carbon Ledger uses enterprise-grade security including end-to-end encryption, secure wallet connections, and follows GDPR and SOC 2 Type II standards. We never store your private keys.',
  'blockchain': 'Carbon Ledger is built on Ethereum with smart contracts for secure, transparent carbon credit transactions. We also integrate with other EVM-compatible networks.',
  ...
  'developer': 'Carbon Ledger is created by Akshit Tiwari. You can reach out via GitHub: https://github.com/AkshitTiwarii, LinkedIn: https://www.linkedin.com/in/akshit-tiwarii/, or email: akshittiwari29@gmail.com',
  'technology': 'Carbon Ledger is built with Next.js 14, React 18, TypeScript, Tailwind CSS, Google Generative AI (Gemini), MetaMask integration, and ThirdWeb for Web3 connectivity.',
  'help': 'I can help you with questions about Carbon Ledger features, AI tools, carbon trading, portfolio management, security, getting started, and sustainability features. What would you like to know?'
};
...
function getFallbackResponse(message: string): string {
  ...
  return `Thank you for your question about Carbon Ledger! Here's what I can help you with:
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      console.log('Carbon Ledger-chat: API_KEY present?', !!API_KEY);
    } catch { }
    ...
    if (genAI === null && API_KEY) {
      try {
        ...
        console.log('Carbon Ledger-chat: genAI initialized');
      } catch (initErr) {
        // initErr may be unknown - cast to any for safe message access
        console.error('Carbon Ledger-chat: genAI initialization error', (initErr as any)?.message || initErr);
      }
    }

    // If no API key or genAI is not available, use fallback responses
    if (!genAI || !API_KEY) {
      console.log('Carbon Ledger-chat: Using fallback response - no API key available or init failed');
    ...
    try {
      ...
      const prompt = `
You are a helpful AI assistant for Carbon Ledger, a carbon credit trading and sustainability platform. You should answer questions based on the following knowledge base and be helpful, informative, and professional.

${Carbon Ledger_KNOWLEDGE_BASE
    }

Context: ${ context || 'General Carbon Ledger Support' }

User Question: ${ message }

    Instructions:
    - Answer questions specifically about Carbon Ledger features, functionality, and capabilities
      - Be helpful and provide detailed explanations when needed
        - If asked about something not related to Carbon Ledger, politely redirect to Carbon Ledger topics
          - Mention specific features and tools when relevant
            - Be encouraging about sustainability and environmental impact
              - Keep responses conversational but professional
                - If you don't know something specific about Carbon Ledger, say so and suggest they contact support
...
Please provide a helpful response:
    `;
      ...
      for (const candidate of candidateModels) {
        try {
          ...
          console.log('Carbon Ledger-chat: attempting model', candidate);
          ...
          if (typeof upstreamStatus === 'number' && upstreamStatus >= 400) {
            console.warn('Carbon Ledger-chat: model', candidate, 'returned status', upstreamStatus);
            if (upstreamStatus === 404) continue;
            continue;
          }
          console.log('Carbon Ledger-chat: model selected', candidate);
          break;
        } catch (mErr) {
          console.warn('Carbon Ledger-chat: model', candidate, 'failed:', (mErr as any)?.message || mErr);
          result = null;
          continue;
        }
      }
    ...
    if (listResp.ok) {
      ...
      console.log('Carbon Ledger-chat: discovered models from ListModels:', remoteModels.join(', '));
      for (const remote of remoteModels) {
        if (candidateModels.includes(remote)) continue;
        try {
          selectedModelInstance = genAI.getGenerativeModel({ model: remote });
          console.log('Carbon Ledger-chat: attempting discovered model', remote);
          const r = await selectedModelInstance.generateContent(prompt);
          const upstreamStatus = r?.response?.status ?? r?.status;
          if (typeof upstreamStatus === 'number' && upstreamStatus >= 400) {
            console.warn('Carbon Ledger-chat: discovered model', remote, 'returned status', upstreamStatus);
            continue;
          }
          result = r;
          console.log('Carbon Ledger-chat: discovered model selected', remote);
          break;
        } catch (dmErr) {
          console.warn('Carbon Ledger-chat: discovered model', remote, 'failed:', (dmErr as any)?.message || dmErr);
          continue;
        }
      }
    } else {
      console.warn('Carbon Ledger-chat: ListModels request failed with status', listResp.status);
    }
  } catch (listErr) {
    console.warn('Carbon Ledger-chat: ListModels error:', (listErr as any)?.message || listErr);
  }
}

if (!result) {
  console.log('Carbon Ledger-chat: no working model found, using fallback');
  const fallbackResponse = getFallbackResponse(message);
...
return NextResponse.json({ response: responseText, mode: 'ai' });
  ...
  console.error('Error in Carbon Ledger chat API:', error);
